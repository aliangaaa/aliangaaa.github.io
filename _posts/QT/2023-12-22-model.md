...
- setindexwidget
在 Qt 中，QTreeView 并没有直接提供 setIndexWidget 函数。setIndexWidget 函数通常用于 QTableWidget 或者 QListWidget 等部件，用于在特定单元格或项上设置自定义的部件。
但是，您可以通过使用 QTreeView 的委托（delegate）来实现类似的功能。委托是 Qt 中用于自定义部件的一种机制，它可以让您为特定的单元格或项提供自定义的绘制和编辑行为。
- setitem
